...
class Box:
    ...
    def x_p(self) -> array:
        x = Jacobi(linspace(-self.L/2, self.L/2, self.Nx)).inverse_map()
        y_r = Jacobi(linspace(0, -self.D, self.Ny)).inverse_map()
        y_l = y_r[1:]
    ...
    def x_m(self) -> array:
        x = Jacobi(linspace(-self.L/2, self.L/2, self.Nx)).inverse_map()
        y_l = Jacobi(linspace(0, -self.D, self.Ny)).inverse_map()
        y_r = y_l[1:]
