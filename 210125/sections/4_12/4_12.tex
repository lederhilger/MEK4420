\begin{figure}[H]
    \centering
    \includegraphics{right_body.pdf}
\end{figure}
We consider a submerged body in an unbounded fluid moving with velocity $\symbf{U} = U(t)\ihat$.
...
\begin{equation}\tag{3.11}\label{eq:REYNOLDS_transport}
    I^{\prime}(t) = \int_{\Omega} \deet f \,\dee V + \int_{\partial\Omega} f \symbf{U}\cdot\nhat \,\dee S,
\end{equation}
...
Due to impermeability, we have that $\symbf{U} \cdot \nhat$ on that contour.
...
\begin{equation*}
    \begin{aligned}
        \symbf{F} = & - \varrho \frac{\dee}{\dee t} \int_{S_B} \phi \nhat \,\dee S\\
...
\[
    \int_{\Omega} \nabla\times\symbf{Q} \,\dee V = \int_{\partial\Omega} \nhat\times\symbf{Q} \,\dee S
\]
to show that
\begin{equation*}
    \begin{aligned}
        \symbf{M} = & - \varrho \frac{\dee}{\dee t}\int_{S_B} \phi (\xvec\times\nhat) \,\dee S\\
