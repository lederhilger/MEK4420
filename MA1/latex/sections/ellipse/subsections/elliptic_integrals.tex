...
We call $\kappa = k^2$ the parameter, and introduce the complementary parameter ${\kappa}^{\prime} = 1 - \kappa$---with it the notation that $\ellK^{\prime}(k) = \ellK(k^{\prime})$, and $\ellE^{\prime}(k) = \ellE(k^{\prime})$---$k$ is named the modulus and $k^{\prime} = \sqrt{\kappa^{\prime}}$ is its complement.
