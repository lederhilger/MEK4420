Recalling now our assumed property of independence of modes in the potential, that we may write it as the superposition $\Phi(t;\xvec) = \symbf{U}(t)\cdot\symbf{\phi}(\xvec)$, we impose the boundary condition of impermeability---no fluid shall cross the boundary $\partial\Omega$.
...
\[
\nhat \cdot \nabla \Phi = \symbf{U} \cdot \nhat, \qquad \text{on } \partial\Omega.
\]
By the product rule, and by virtue of $\symbf{U}$ not being a function of the spatial variable, we have through the commutativity of the inner product that $\symbf{U} \cdot \big( \nhat \cdot \nabla\symbf{\phi}(\xvec) \big) = \symbf{U} \cdot \nhat.$
